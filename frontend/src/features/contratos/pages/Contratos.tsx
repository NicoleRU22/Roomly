          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              <div className="space-y-1.5">
                <label className="block font-bold text-slate-650">1. Sube foto de tu firma manuscrita</label>
                <input
                  type="file"
                  accept="image/*"
                  onChange={handleSignatureFileChange}
                  className="w-full text-xs file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-[10px] file:font-bold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100 transition-colors"
                />
                <p className="text-[10px] text-slate-400">Firma en una hoja blanca, tómale foto y súbela aquí.</p>
              </div>

              {signatureBase64 && (
                <div className="border border-slate-100 p-2.5 rounded-2xl bg-slate-50 text-center">
                  <span className="text-[10px] font-bold text-slate-500 block">Vista previa de tu firma</span>
                  <img src={signatureBase64} alt="Firma cargada" className="h-12 mx-auto object-contain mt-1.5 rounded border bg-white" />
                </div>
              )}
            </div>
